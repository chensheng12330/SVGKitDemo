...
- (void)dealloc
{
    NSLog(@"====> SVGKFastImageView-> dealloc");
...
-(void)drawRect:(CGRect)rect
{
    ...
    SEL act = @selector(willDrawSVGView);
    if ([self.delegate respondsToSelector:act]) {
        [self.delegate performSelector:act];
    }
	...
	for( int k=0; k<rows; k++ )
    {
		for( int i=0; i<cols; i++ )
		{
			...
			self.image.CALayerTree.contents = nil;
            
			CGContextRestoreGState(context);
		}
    }
    
    act = @selector(didDrawSVGView);
    if ([self.delegate respondsToSelector:act]) {
        [self.delegate performSelector:act];
    }
