...
- (void)dealloc
{
    NSLog(@"=====> SVGKImageCacheLine - dealloc. ");
...
{
    return [NSString stringWithFormat:@"numberOfInstances: %d SVGKImage_RetainCount:%d", _numberOfInstances,_mainInstance.retainCount];
}
@end
#endif
...
+(SVGKImage*)imageWithContentsOfFileOrCache:(NSString *)aPath
{
    ...
    NSString *name = [SVGKImage md5:aPath];
	...
	SVGKImage* result = [self imageWithContentsOfFile:aPath];
...
- (void)dealloc
{
    NSLog(@" [SVGKImage dealloc] %@ -> %d", globalSVGKImageCache, globalSVGKImageCache.count);
...
- (void)  releaseGlobalSVGKImageCache
{
    [globalSVGKImageCache removeAllObjects];
    [globalSVGKImageCache release];
    globalSVGKImageCache = nil;
}

+ (void) releaseGlobalSVGKImageForId:(NSString*) identify
{
    SVGKImageCacheLine* cacheLine = [globalSVGKImageCache valueForKey:identify];
    if (cacheLine) {
        NSLog(@"===> release for SVGKImgae: %@",cacheLine);
        [globalSVGKImageCache removeObjectForKey:identify];
    }
    return;
}
...
- (CALayer *)newLayerWithElement:(SVGElement <SVGLayeredElement> *)element
{
    ...
    SEL act = @selector(SVGKParseForCAlayer:);
	...
	for (SVGElement *child in childNodes )
	{
        if (element== self.DOMTree) {
            if ([self.delegate respondsToSelector:act]) {
                [self.delegate performSelector:act withObject:layer];
            }
        }
		...
		if ([child conformsToProtocol:@protocol(SVGLayeredElement)]) {
            ...
            //NSLog(@"add sublayer: %@，  %@", sublayer.name,NSStringFromClass([sublayer class]));
		}
...
-(CALayer *)CALayerTree
{
	if( CALayerTree == nil )
	{
        ...
        self.bakTree     = self.CALayerTree;
        
        /////////***  all layer complete.
        SEL action = @selector(completeAddAllLayer:);
