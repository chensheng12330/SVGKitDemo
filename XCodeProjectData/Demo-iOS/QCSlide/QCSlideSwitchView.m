...
@interface QCSlideSwitchView ()

@property (nonatomic, retain) NSMutableArray *arBtns;
@end
...
- (void)initValues
{
    ...
    self.panTap = @selector(scrollHandlePan:);
}
...
- (void)setRigthSideButton:(UIButton *)rigthSideButton
{
    ...
    [_rigthSideButton addTarget:self action:@selector(moveTopViewToRight) forControlEvents:UIControlEventTouchUpInside];
...
- (void)buildUI
{
    ...
    for (int i=0; i<number; i++) {
        BOOL bAddTap=NO;
        UIViewController *vc = [self.slideSwitchViewDelegate slideSwitchView:self viewOfTab:i isAddTap:&bAddTap];
        if (bAddTap==YES) {
            /*[_rootScrollView.panGestureRecognizer addTarget:self action:@selector(scrollHandlePan:)];
            UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(scrollHandlePan:)];
    ...
    if (self.slideSwitchViewDelegate && [self.slideSwitchViewDelegate respondsToSelector:@selector(slideSwitchView:didselectTab:)]) {
        NSLog(@"buildUI");
        //[首次加载，暂时注]   [self.slideSwitchViewDelegate slideSwitchView:self didselectTab:_userSelectedChannelID - 100];
    }
...
- (void)createNameButtons
{
    ...
    self.arBtns = [[NSMutableArray alloc]init];
...
#pragma mark - Mark outlet method
-(void) goToViewForIndex:(int) index
{
    if (index<0 || index>=self.arBtns.count) {
        DLog(@"数组越界");
        return;
    }
    UIButton *btn = self.arBtns[index];
    NSLog(@"selectNameButton");
    [self selectNameButton:btn];
    return;
}

-(void) moveTopViewToRight
{
    
    CGFloat offR = kWidthOfButtonMargin;
    if (self.arBtns.count>0) {
        offR = ((UIView*) self.arBtns[0]).frame.size.width + 2;
    }
    
    CGPoint point = _topScrollView.contentOffset;
    
    if ( (point.x + offR) >= offR*self.arBtns.count) {
        return;
    }
    
    [_topScrollView setContentOffset:CGPointMake(point.x + offR, 0)  animated:YES];
}
